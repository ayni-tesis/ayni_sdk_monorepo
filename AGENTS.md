...
- SDK credentials: administrator- and owner-only generation and regeneration of credentials scoped to one application. The server stores only the SHA-256 hash of the secret, reveals the secret once at creation or regeneration, rejects non-active credentials or archived applications, and immediately revokes the prior credential upon regeneration.
