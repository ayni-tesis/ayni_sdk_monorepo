...
- Workflow canvas: the dashboard uses `@dnd-kit/react` for draggable nodes, palette drops, and typed visual connections; free positions persist in `draft.layout` and are editable only by administrators of active applications.
...
- **Publish only validated drafts (US-036)**: US-035's `validateWorkflowDraft`
  (`apps/server/src/workflow-validation.ts`) is exposed only as a read-only check.
  Publishing must run it on the draft read under the workflow row lock and refuse
  when `publishable` is false, so the published DAG is the validated one.
- **Published workflow versions (US-036)**: `getWorkflow`
  (`apps/server/src/workflow-store.ts`) still returns `versions: []`, typed
  `never[]` until version storage exists. `workflow-detail-view.tsx` renders the
  empty state in `Versiones publicadas`; US-036 fills and renders that content.
