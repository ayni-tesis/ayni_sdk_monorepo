...
- Workflow canvas: the dashboard uses `@dnd-kit/react` for draggable nodes, palette drops, and typed visual connections; administrators of active applications can select and delete draft nodes after confirmation, which removes incident connections and transitive condition/output dependents without changing published versions; free positions persist in `draft.layout` and are editable only by administrators of active applications.
