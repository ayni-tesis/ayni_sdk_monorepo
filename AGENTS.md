...
- Workflow version download (US-041): `GET /sdk/workflow-versions/:workflowVersionId` authenticates an SDK credential and returns only the immutable definition of a version owned by an active application and non-archived workflow; all unavailable cases are the same 404. During `AyniSdk.sync()`, new manifest workflow versions are downloaded to isolated temporary files and reported through progress/result callbacks; unavailable downloads leave the local inventory unchanged. The downloader sends credentials only over HTTPS, or explicitly enabled loopback HTTP.
...
- US-131 reassigns the source of a condition or an output. The port rules live once in `@ayni/api/workflow-graph` (`workflowOutputPortType`, `areWorkflowPortsCompatible`, `isConditionSourceCompatible`, `isOutputSourceCompatible`, `workflowSourceTarget`, `workflowPortCompatibility`), used by the server and the dashboard alike. The `Origen` input always has one source: `workflowPortCompatibility` accepts on it a classification model `Resultado` producing a condition's label, or for an output a model `Resultado` of its `Tipo de resultado` (a `Verdadero` or `Falso` branch when it is boolean), and reports its current source as `connected`. `POST …/connections` to a condition's or output's `source` input goes through `changeWorkflowDraft` like any change: it locks the row, checks the rules and the cycle rule over every edge of the draft, and replaces only the node's `sourceNodeId` (and an output's `sourcePort`) in one write, keeping its id, settings, and position and adding no stored connection; an unaccepted source answers 409 `code: "incompatibleSource"` with `Esta condición no es compatible con la salida seleccionada.` or `El resultado seleccionado no es compatible con la salida.` Administrators of active applications drop another output on the `Origen` (or use `Salida <puerto>` then `Conectar origen de <nodo>`), or drag the source end of the current edge (the only reconnectable edges, `reconnectable: "source"`) to another output, which marks the outputs that input accepts; a drop on the background or back on the current source leaves the edge. The new edge replaces the old one at once, dotted (`1 6`) while it saves, then shows `Conexión actualizada.`; an unaccepted source is refused in the browser with the node's message, and any failure puts the previous source back with that message (server `incompatibleSource`) or `No pudimos actualizar la conexión.` (a `draftConflict` shows the US-130 notice instead). Published versions never change.
